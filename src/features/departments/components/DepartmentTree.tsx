...
interface DepartmentTreeProps {
  departments: IDepartmentDb[];
  setFocusedDepartment: (department: IDepartmentDb | null) => void;
  setHasChildren: (hasChildren: boolean) => void;
}

const DepartmentTree: FC<DepartmentTreeProps> = ({
  departments,
  setFocusedDepartment,
  setHasChildren,
}) => {
  const handleFocusedDepartment: DirectoryTreeProps['onSelect'] = (
    keys,
    info
  ) => {
    const focusedDepartmentId: string | number | null = keys.length
      ? keys[0]
      : null;

    const focusedDepartment: IDepartmentDb | null =
      find(departments, ['id', focusedDepartmentId]) || null;
